...
<div class="tab-content">
    ...
    <div class="tab-content__pane" id="profile">
    	<div class="intro-y datatable-wrapper overflow-x-auto">
		    <table class="table table-report table-report--bordered display datatable w-full" id="compras_rechazadas">
		        <thead>
		            <tr>
		                <th class="border-b-2 whitespace-no-wrap">Id</th>
		                <th class="border-b-2 text-center whitespace-no-wrap">Descripcion</th>
...
<script type="text/javascript">
    $(function () {
      ...
      var table = $('#compras_realizadas').DataTable({
          processing: true,
          serverSide: true,
        ajax: "{{ route('gerencia.comprascreadas') }}",
        columns: [
...
<script type="text/javascript">
    $(function () {
      ...
      var table = $('#compras_autorizadas').DataTable({
          processing: true,
          serverSide: true,
        ajax: "{{ route('gerencia.autorizadas') }}",
        columns: [
            {data: 'id', name: 'id', class:"border-b"},
            {data: 'descripcion', name: 'descripcion', class: "text-center border-b"},
            {data: 'formadepago', name: 'formadepago', class: "text-center border-b"},
...
<script type="text/javascript">
    $(function () {
      $('#compras_rechazadas').DataTable().destroy();
      var table = $('#compras_rechazadas').DataTable({
          processing: true,
          serverSide: true,
        ajax: "{{ route('gerencia.rechazarcompra') }}",
        columns: [
            {data: 'id', name: 'id', class:"border-b"},
            {data: 'descripcion', name: 'descripcion', class: "text-center border-b"},
            {data: 'formadepago', name: 'formadepago', class: "text-center border-b"},
