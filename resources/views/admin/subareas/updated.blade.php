<div class="modal" id="edit-subarea{{$subarea->id}}">
    <div class="modal__content modal__content--xl">
        <div class="flex items-center px-5 py-5 sm:py-3 border-b border-gray-200">
            <h2 class="font-medium text-base mr-auto">
                Actualizacion del subáreas: {{$subarea->nombre}}
            </h2>
            <!--
        ...
        {!! Form::open(['route' => ['subareas.update', $subarea->id], 'method' => 'PUT']) !!}
        {{ csrf_field() }}
            <div class="p-5 grid grid-cols-12 gap-4 row-gap-3">
                <div class="col-span-12 sm:col-span-4 @if ($errors->has('clave')) has-error @endif">
                    ...
                    <input type="text" class="input w-full border mt-2 flex-1" name="clave" id="clave" value="{{ $subarea->clave}}" aria-describedby="helpClave">
                ...
                <div class="col-span-12 sm:col-span-4 @if ($errors->has('nombre')) has-error @endif">
                    <label class="font-medium text-base mr-auto" for="nombre">Nombre del subárea:</label>
                    <input type="text" class="input w-full border mt-2 flex-1" name="nombre" id="nombre" value="{{ $subarea->nombre }}" aria-describedby="helpNombre">
                ...
                <div class="col-span-12 sm:col-span-4 @if ($errors->has('area_id')) has-error @endif">
                    <label class="font-medium text-base mr-auto" for="area_id">Nombre del area:</label>
                    <select class="input w-full border mt-2 flex-1" id="area_idcreate" name="area_id">
                        @foreach($areas as $area)
                            <option value="{{$area->id}}" @if($subarea->area->id == $area->id) selected="selected" @endif>{{$area->nombre}} - {{$area->empresa->nombre}} </option>
                        @endforeach
                    </select>
                </div>
