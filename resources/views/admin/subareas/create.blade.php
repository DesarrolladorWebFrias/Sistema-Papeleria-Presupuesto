<div class="modal" id="crear_subarea">
    <div class="modal__content modal__content--xl">
        ...
        <form method="POST" action="{{ route('subareas.store') }}"  role="form">
            {{ csrf_field() }}
            <div class="p-5 grid grid-cols-12 gap-4 row-gap-3">
                 <div class="col-span-12 sm:col-span-4 @if ($errors->has('clave')) has-error @endif">
                    ...
                    <input type="text" class="input w-full border mt-2 flex-1" name="clave" id="clave" aria-describedby="helpClave">
                ...
                <div class="col-span-12 sm:col-span-4 @if ($errors->has('area_id')) has-error @endif">
                    ...
                    <select class="input w-full border mt-2 flex-1" id="area_id" name="area_id">
                    		<option value="">Selecciona una opcion</option>
                        @foreach($areas as $area)
                            <option value="{{$area->id}}">{{$area->nombre}} - {{$area->empresa->nombre}}</option>
                        @endforeach
                    </select>
                </div>
